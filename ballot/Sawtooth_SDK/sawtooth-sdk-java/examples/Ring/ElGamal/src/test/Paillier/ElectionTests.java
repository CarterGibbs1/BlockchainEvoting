...
public class ElectionTests {
    ...
    @Test
    public void outputElectionKeys() throws IOException {
        // Public Key
        File f = new File("ElectionPubKey.txt");
        if (f.exists() || f.isFile()) f.delete();
        f.createNewFile();
        FileWriter fw = new FileWriter(f);
        PrintWriter pw = new PrintWriter(fw);
        Election e = new Election();
        pw.print(e.getPk().toString());
        pw.close();
        fw.close();
        // Private Key
        f = new File("ElectionPrivKey.txt");
        if (f.exists() || f.isFile()) f.delete();
        f.createNewFile();
        fw = new FileWriter(f);
        pw = new PrintWriter(fw);
        pw.print(e.getSk().toString());
        pw.close();
        fw.close();
    }
